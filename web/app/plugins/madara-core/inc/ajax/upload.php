...
class WP_MANGA_AJAX_UPLOAD{
    ...
    function import_album_chapter(){
        ...
        $slugified_name = $wp_manga_storage->slugify( $_POST['name'] );

        $chapter_args = array(
			'post_id'             => $_POST['post'],
			'volume_id'           => !empty( $_POST['volume'] ) ? $_POST['volume'] : '0',
    ...
    function wp_manga_upload_chapter() {
        ...
        if( !empty( $chapter_id ) ) {
            if( isset( $chapter_id['error'] ) ) {

                wp_send_json_error( $chapter_id );

			}else if( is_wp_error( $chapter_id ) ){

                wp_send_json_error( array(
                    'error' => true,
                    'messsage' => $chapter_id->get_error_message()
                ) );
