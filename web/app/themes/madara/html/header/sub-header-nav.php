<?php
	...
	if ( $has_secondary_menu || ($user_enabled || $user_manga_logged) ) {
		?>
        <div class="<?php echo $has_secondary_menu ? '' : 'no-subnav';?> c-sub-header-nav<?php echo esc_attr( $header_bottom_border == 'on' ? ' with-border ' : '' ); ?> <?php echo esc_attr($sticky_menu == 0 ? 'hide-sticky-menu' : ''); ?>">
            <div class="container <?php echo esc_attr( $madara_header_style == '2' ? 'custom-width' : '' ); ?>">
                <div class="c-sub-nav_wrap">
                    <div class="sub-nav_content">
                        <ul class="sub-nav_list list-inline second-menu">
							<?php
								...
								?>	
								<i class="mobile-icon ion-more"></i>
								<?php
								}
							?>
					...
					<?php } elseif ( $user_manga_logged ) { ?>
                        <div class="c-modal_item">
							<?php
								echo $wp_manga_user_actions->get_user_section();
							?>
                        </div>
					<?php } ?>
